...
File.open("vtparse_enums.h", "w") { |f|
    f.puts "#ifndef _VTPARSE_ENUMS_H_"
    f.puts "#define _VTPARSE_ENUMS_H_ 1"
    f.puts "typedef enum {"
    ...
    f.puts "#endif /* _VTPARSE_ENUMS_H_ */"
}

puts "Wrote vtparse_enums.h"

File.open("vtparse_table.h", "w") { |f|
    f.puts "#ifndef _VTPARSE_TABLE_H_"
    f.puts "#define _VTPARSE_TABLE_H_ 1"
    f.puts "#include \"vtparse_enums.h\""
    f.puts
    ...
    f.puts
    f.puts "#endif /* _VTPARSE_TABLE_H_ */"
}

puts "Wrote vtparse_table.h"

File.open("vtparse_names.c", "w") { |f|
    f.puts
    f.puts "static char *ACTION_NAMES[] = {"
    f.puts "   \"<no action>\","
    ...
    f.puts "};"
    f.puts
    f.puts "static __attribute__((unused)) char *STATE_NAMES[] = {"
    f.puts "   \"<no state>\","
